...
AES_KEY="$(echo "$AES_KEY_ENCRYPTED" | openssl $CIPHER -salt -a -A -d -pass file:<(echo -n "$AES_PASS"))"
if [ "$?" != "0" ] ; then
	echo "Error while decypting AES key." 1>&2
	exit 1
fi
tail -n +$SELF_LINES "$0" | gzip -d | tail -n+2 | openssl $CIPHER -salt -a -A -d -pass file:<(echo -n "$AES_KEY")
exit
...
get_public_key () {
	...
	echo "Loading public key..." > /dev/tty
	otpme-user $OTPME_OPTS dump_key "$USERNAME" | base64 -d
}
...
dump_private_key () {
	...
	echo "Loading private key..." > /dev/tty
...
aes_encrypt () {
	...
	cat | openssl $CIPHER -salt -a -A -pass file:<(echo -n "$PASSWORD_SALT$AES_PASS") | base64 -w 0
...
aes_decrypt () {
	...
	cat | openssl $CIPHER -salt -a -A -d -pass file:<(echo -n "$PASSWORD_SALT$AES_PASS")
	if [ "${PIPESTATUS[1]}" != "0" ] ; then
...
decrypt_file () {
	...
	if [ "$ENC_TYPE" = "AES" ] ; then
		...
		if [ "$KEY_ENC_TYPE" = "rsa" ] ; then
			if [ "$SIGN_MODE" = "server" ] ; then
				...
				# Decrypt PKCS1_v1_5
				# openssl rsautl -decrypt -inkey /dev/stdin -in
				# Decrypt PKCS1_OAEP
				# openssl rsautl -oaep -decrypt -inkey /dev/stdin -in
				AES_KEY="$(echo "$AES_KEY_ENCRYPTED" | base64 -d | openssl rsautl -oaep -decrypt -inkey <(echo -n "$PRIVATE_KEY") -in /dev/stdin)"
			fi
		fi
		# Finally decrypt AES data.
		base64 -d | openssl enc -d -$CIPHER -in /dev/stdin -pass file:<(echo -n "$AES_KEY") > "$OUTFILE"
	fi
	...
	if [ "$ENC_TYPE" = "RSA" ] ; then
		if [ "$SIGN_MODE" = "server" ] ; then
			...
			PRIVATE_KEY="$(get_private_key)"
			base64 -d | openssl rsautl -oaep -decrypt -inkey <(echo -n "$PRIVATE_KEY") -in /dev/stdin > "$OUTFILE"
		fi
	fi
}
...
case "$COMMAND" in
	...
	gen_keys)
		echo "Generating key ($KEY_LEN)..." > /dev/tty
	...
	sign)
		...
		if [ "$SIGN_MODE" = "server" ] ; then
			...
			if [ "$_OTPME_KEYSCRIPT_KEY_PASS" = "" ] ; then
				if ! SIGNATURE="$(otpme-user $OTPME_OPTS sign --digest "$SHA1_SUM" "$_OTPME_KEYSCRIPT_USER")" ; then
					exit 1
				fi
			else
				if ! SIGNATURE="$(echo "$_OTPME_KEYSCRIPT_KEY_PASS" | $(which otpme-user) $OTPME_OPTS sign --digest "$SHA1_SUM" --stdin-pass "$_OTPME_KEYSCRIPT_USER")" ; then
					exit 1
				fi
			fi
		else
	...
	encrypt)
		...
		if [ "$AES_KEY_ENC" = "rsa" ] ; then
			...
			# Encrypt PKCS1_v1_5
			# openssl rsautl -encrypt -inkey file:<(echo -n "$PUBLIC_KEY") -pubin
			# Encrypt PKCS1_OAEP
			# openssl rsautl -encrypt -oaep -inkey file:<(echo -n "$PUBLIC_KEY") -pubin
			if [ "$ENC_TYPE" = "RSA" ] ; then
				cat "$FILE" | openssl rsautl -oaep -encrypt -inkey <(echo -n "$PUBLIC_KEY") -pubin | base64 -w 0 > "$OUTFILE"
				# We need a newline to get the first line with bash's read builtin in handle_file_type()
				echo -en "\n" >> "$OUTFILE"
				exit $?
			else
				AES_KEY="$(openssl rand -base64 32)"
				AES_KEY_ENCRYPTED="$(echo "$AES_KEY" | openssl rsautl -oaep -encrypt -inkey <(echo -n "$PUBLIC_KEY") -pubin | base64 -w 0)"
			fi
		else
			AES_KEY="$(openssl rand -base64 32)"
		...
		(echo "$AES_KEY_ENCRYPTED";cat "$FILE" | openssl enc -$CIPHER -salt -pass file:<(echo -n "$AES_KEY") | base64 -w 0) | gzip -f >> "$OUTFILE"
	;;
