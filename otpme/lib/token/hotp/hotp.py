...
class HotpToken(OathToken):
    ...
    def gen_otp(
        ...
        **kwargs,
        ...
        if not secret:
            return callback.error(_("Unable to get token secret."))
        ...
        **kwargs,
        ...
        if hotp_status:
            ...
            if hotp_count > token_counter:
                ...
                # True, like every other token type. This used to hand
                # the OTP back, and no caller ever read it as anything
                # but a yes -- it only carried the PIN and the OTP in
                # clear text further up the stack than they need to go.
                # The MSCHAP path below is the one that does need the
                # value, and returns it in its tuple.
                return True
    ...
    def gen_qrcode(
        ...
        **kwargs,
        ...
        # Gen OATH URI.
        user_string = f"{self.rel_path}@{self.realm}"
    ...
    def resync(
        ...
        **kwargs,
        ...
        if len(otp) < self.otp_len:
            return callback.error(_("OTP too short."))
        if len(otp) > self.otp_len:
            return callback.error(_("OTP too long."))
        ...
        hotp_count = hotp.verify_hotp(counter_start=token_counter_start,
                                        ...
                                        otp=otp)
        if not hotp_status:
            return callback.error(_("Unable to synchronize token."))
        ...
        try:
            ...
            msg = _("Error adding OTP to list of used OTPs: {e}")
            msg = msg.format(e=e)
            return callback.error(msg)
        ...
        msg = msg.format(hotp_count=hotp_count)
        callback.send(msg)
    ...
    def _check_range_format(
        ...
        if not isinstance(check_range, int):
            msg = _("Counter check range must be an integer.")
            return callback.error(msg)
        if check_range > 0:
            return callback.ok()
    ...
    def deploy(
        ...
        if not self.check_pin(pin=pin, callback=callback):
            return callback.error(_("Invalid PIN."))

        msg = _("Setting received server secret to token: {self.rel_path}")
