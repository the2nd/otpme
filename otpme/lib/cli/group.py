    ...
    for group_uuid in group_order:
        ...
        if get_tokens:
            ...
            if group_tokens_count > max_tokens:
                msg = _("({tokens_len} of {group_tokens_count} tokens total)")
                msg = msg.format(tokens_len=len(processed_tokens), group_tokens_count=group_tokens_count)
                x = msg
        ...
        if get_sync_users:
            ...
            if group_users_count > max_sync_users:
                msg = _("({users_len} of {group_users_count} users total)")
                msg = msg.format(users_len=len(processed_users), group_users_count=group_users_count)
                x = msg
