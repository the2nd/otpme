...
register_global_opt("--no-pager", "Do use pager (less) on command output larger than terminal size.")
...
def get_help(command, subcommand=None, command_map=None,
    ...
    if not command in command_map:
        msg = _("Command not in command map: {command}")
    ...
    for c in sorted(cmd_map):
        ...
        for f in command_map[command][mod_name][c]['_help']:
            if f == "cmd":
                help_text = f"\t- {command_map[command][mod_name][c]['_help'][f]}"
                row = [ f" {c}", help_text ]
...
def _get_main_opts(clear_cache=False, mod_name=None):
    ...
    for x in list(sys.argv):
        ...
        if sys.argv[0].startswith("-d"):
            ...
            sys.argv.pop(0)
        elif sys.argv[0] == "--no-pager":
            main_opts['use_pager'] = False
            sys.argv.pop(0)
