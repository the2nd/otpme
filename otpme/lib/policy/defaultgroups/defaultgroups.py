...
class DefaultgroupsPolicy(Policy):
    ...
    def remove_orphans(
        ...
        **kwargs,
        ...
        """ Remove orphan UUIDs. """
        extra_ref_lists = [
                ('default_groups', 'group', None),
                ('default_group', 'group', None, 'value'),
                ]
                ...
                if result:
                    ...
                    for group_uuid in self.default_groups:
                        ...
                        try:
                            ...
                            msg = _("Failed to add default token to group: {group_name}: {default_token}, {error}")
                            msg = msg.format(group_name=group.name, default_token=default_token, error=e)
                            return callback.error(msg)
        else:
    ...
    def show_config(self, callback=default_callback, **kwargs):
        ...
        if self.verify_acl("view:default_group"):
            if self.default_group is not None:
                ...
                if result:
                    default_group = result[0]
        lines.append(f'DEFAULT_GROUP="{default_group}"')
        ...
        or self.verify_acl("remove:default_group"):
            ...
            groups_string = f"{','.join(defaultgroups)}"
            default_groups.append(groups_string)
        lines.append(f'DEFAULT_GROUPS="{",".join(default_groups)}"')
