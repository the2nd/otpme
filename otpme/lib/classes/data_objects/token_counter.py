...
REGISTER_BEFORE = []
REGISTER_AFTER = ["otpme.lib.classes.user"]
#REGISTER_AFTER = []
...
def register_oid():
    ...
    # Separator escaped, see accessgroup.py. The last part is the object
    # hash from the schema above, not a counter value -- "[0-9]*" never
    # matched one.
    #token_counter_oid_re = f'token_counter|{realm_name_re}[/]{site_name_re}[/]{oid.uuid_re}[/][0-9]*'
    token_counter_oid_re = (f'token_counter[|]{realm_name_re}[/]{site_name_re}'
                        f'[/]{oid.uuid_re}[/][a-f0-9]+')
    oid.register_oid_schema(object_type="token_counter",
                            full_schema=full_oid_schema,
                            read_schema=read_oid_schema,
                            oid_regex=token_counter_oid_re)
    rel_path_getter = lambda x: x[-3:]
    oid.register_rel_path_getter(object_type="token_counter",
                                getter=rel_path_getter)

def register_sync_settings():
    config.register_cluster_sync(object_type="token_counter")

def register_backend():
    """ Register object for the file backend. """
    path_id = "token_counter"
    ...
    def oid_getter(path):
        ...
        object_id = oid.OTPmeOid(object_type="token_counter",
                                realm=object_realm,
                                site=object_site,
        ...
        for user_uuid in filetools.list_dir(used_dir):
            ...
            for token_uuid in filetools.list_dir(user_used_dir):
                if not stuff.is_uuid(token_uuid):
                    continue
                token_counter_dir = os.path.join(user_used_dir, token_uuid)
                token_counter_files = filetools.list_dir(token_counter_dir)
                counter = 0
                files_count = len(token_counter_files)
    ...
    config.register_object_type(object_type="token_counter",
                            tree_object=False,
                            uniq_name=False,
                            add_after=["token"],
                            object_cache=1024,
...
class TokenCounter(UsedHash):
    """ Class that implements used SLP. """
