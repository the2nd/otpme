...
def register_oid():
    ...
    used_slp_oid_re = ('used_slp|%s[/]%s[/]%s[/][a-f0-9]*'
                    % (realm_name_re, site_name_re, oid.uuid_re))
    oid.register_oid_schema(object_type="used_slp",
                            full_schema=full_oid_schema,
                            read_schema=read_oid_schema,
                            oid_regex=used_slp_oid_re)
    rel_path_getter = lambda x: x[2:]
    oid.register_rel_path_getter(object_type="used_slp",
                                getter=rel_path_getter)

def register_sync_settings():
    config.register_cluster_sync(object_type="used_slp")
...
def register_backend():
    ...
    def upath_getter(object_id):
        try:
            ...
            used_slp_dir = os.path.join(user_used_dir, path_id)
        except:
            return
        return used_slp_dir
    ...
    def oid_getter(path):
        ...
        object_id = oid.OTPmeOid(object_type="used_slp",
                                realm=object_realm,
                                site=object_site,
                                user_uuid=user_uuid,
    ...
    def index_rebuild():
        for user_uuid in filetools.list_dir(used_dir):
            ...
            used_slp_dir = os.path.join(user_used_dir, path_id)
            used_slp_files = filetools.list_dir(used_slp_dir)
            counter = 0
            files_count = len(used_slp_files)
...
class UsedSLP(UsedHash):
    """ Class that implements used SLP. """
    def __init__(self, **kwargs):
        # Set our type (used in parent class).
        self.type = "used_slp"
        # Call parent class init.
        super(UsedSLP, self).__init__(**kwargs)
