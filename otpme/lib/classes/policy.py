...
def get_value_acls(split=False, **kwargs):
    ...
    config_params = config.get_config_parameters("policy")
    if split:
...
def register():
    register_oid()
    config.register_config_parameter(name="max_policy_name_len",
                                    ctype=int,
                                    default_value=64,
...
class Policy(OTPmeObject):
    ...
    def rename(
        ...
        **kwargs,
        ...
        """ Rename a token. """
        msg = _("Rename policy '{policy_name}' to '{new_name}'?: ")
        msg = msg.format(policy_name=self.name, new_name=new_name)
