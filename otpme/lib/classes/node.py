...
def register():
    register_oid()
    config.register_config_parameter(name="max_node_name_len",
                                    ctype=int,
                                    default_value=64,
...
def register_oid():
    ...
    node_path_re = f'{unit_path_re}[/]{node_name_re}'
    # An OID is not a path, see accessgroup.py: no leading slash, and
    # the unit part is optional because the read OID has none.
    #node_oid_re = f'node|{node_path_re}'
    node_oid_re = (f'node[|]{realm_name_re}[/]{site_name_re}'
                f'([/]{unit_name_re})*[/]{node_name_re}')
...
def register_hooks():
    ...
    config.register_auth_on_action_hook("node", "change_vote_script")
    config.register_auth_on_action_hook("node", "enable_vote_script")
    config.register_auth_on_action_hook("node", "disable_vote_script")
...
class Node(OTPmeHost):
    ...
    def change_vote_script(
        ...
        **kwargs,
        ...
        msg = _("Change vote script of node '{node_name}' to '{script}'?: ")
        msg = msg.format(node_name=self.name, script=vote_script)
    ...
    def enable_vote_script(
        ...
        **kwargs,
        ...
        msg = _("Enable vote script for node '{node_name}'?: ")
        msg = msg.format(node_name=self.name)
        if not self.ask_change_confirmation(msg, force=force, callback=callback):
            return callback.abort()
    ...
    def disable_vote_script(
        ...
        **kwargs,
        ...
        msg = _("Disable vote script for node '{node_name}'?: ")
        msg = msg.format(node_name=self.name)
        if not self.ask_change_confirmation(msg, force=force, callback=callback):
            return callback.abort()
