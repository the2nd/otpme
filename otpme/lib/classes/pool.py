...
def get_value_acls(split=False, **kwargs):
    ...
    config_params = config.get_config_parameters("pool")
    if split:
...
def register():
    register_oid()
    config.register_config_parameter(name="max_pool_name_len",
                                    ctype=int,
                                    default_value=64,
...
def register_oid():
    ...
    pool_oid_re = (f'pool[|]{realm_name_re}[/]{site_name_re}'
                f'([/]{unit_name_re})*[/]{pool_name_re}')
...
class Pool(OTPmeObject):
    ...
    def rename(
        ...
        **kwargs,
        ...
        """ Rename pool. """
        msg = _("Rename pool '{pool_name}' to '{new_name}'?: ")
        msg = msg.format(pool_name=self.name, new_name=new_name)
