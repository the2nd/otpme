...
def register():
    register_dn()
    register_oid()
    config.register_config_parameter(name="max_group_name_len",
                                    ctype=int,
                                    default_value=32,
...
def register_oid():
    ...
    group_path_re = f'{unit_path_re}[/]{group_name_re}'
    # An OID is not a path, see accessgroup.py. Our read OID is realm
    # and name alone -- no site either -- so site and unit go into one
    # optional part: "group|<realm>/<name>" or
    # "group|<realm>/<site>/<unit>.../<name>".
    #group_oid_re = f'group|{group_path_re}'
    group_oid_re = (f'group[|]{realm_name_re}[/]'
                    f'({site_name_re}[/]({unit_name_re}[/])+)?{group_name_re}')
...
class Group(OTPmeObject):
    ...
    def add_default_group_user(
        ...
        **kwargs,
        ...
        if user_uuid in self.default_group_users:
            msg = _("User already added to group '{group_name}'.")
    ...
    def remove_default_group_user(
        ...
        **kwargs,
        ...
        if user_uuid not in self.default_group_users:
            if ignore_missing:
                return
            msg = _("User not in group '{group_name}'.")
            msg = msg.format(group_name=self.name)
            return callback.error(msg)
    ...
    def list_hosts(
        ...
        **kwargs,
        ...
        """ Return list with hosts valid for this group. """
    ...
    def enable(
        ...
        **kwargs,
        ...
        A group's hosts only count toward a share's
        get_hosts(include_groups=True) result while the group is
        enabled (see share.get_hosts). Enabling the group can
        therefore re-add hosts to the allow-list of every
        limit_by_hosts share that has this group assigned -- users
        on those hosts may now be able to mount. """
        affected_shares = backend.search(object_type="share",
                                         attribute="group",
                                         value=self.uuid,
                                         return_type="instance")
    ...
    def disable(
        ...
        **kwargs,
        ...
        share.get_hosts(include_groups=True) skips disabled groups, so
        disabling this group shrinks the allow-list of every
        limit_by_hosts share that has it assigned. Users on those
        hosts lose mount access and need to be told to unmount. """
        affected_shares = backend.search(object_type="share",
                                         attribute="group",
                                         value=self.uuid,
                                         return_type="instance")
    ...
    def add(
        ...
        **kwargs,
        ...
        if result:
            group_oid = result[0]
            msg = _("Group already exists: {group_oid}")
            msg = msg.format(group_oid=group_oid)
            return callback.error(msg)
    ...
    def move(self,
        ...
        **kwargs,
        ...
        if self.name in internal_groups:
            msg = _("Moving internal group is not allowed.")
            return callback.error(msg)
    ...
    def cross_site_move(
        ...
        **kwargs,
        ...
        if config.use_api:
            msg = _("Cannot do cross-site move in API mode.")
            return callback.error(msg)
    ...
    def delete(
        ...
        **kwargs,
        ...
        """ Delete group. """
        if not self.exists():
            return callback.error(_("Group does not exist."))
        ...
        if self.is_special_object(return_true_false=True):
            msg = _("Cannot delete special group: {group_name}")
            msg = msg.format(group_name=self.name)
            return callback.error(msg)
        ...
        if self.verify_acl("view:roles"):
            ...
            for i in self.roles:
                ...
                if not otpme_acl.access_granted(object_id=role_oid,
                                                acl="view_public:object"):
                    continue
                role_path = f"{role_oid.rel_path} ({role_oid.site})"
