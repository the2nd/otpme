...
class Dictionary(OTPmeObject):
    ...
    def add_words(
        ...
        **kwargs,
        ...
        msg = _("Add {word_count} words to dictionary '{dict_name}'?: ")
        msg = msg.format(word_count=len(word_list), dict_name=self.name)
        if not self.ask_change_confirmation(msg, force=force, callback=callback):
            return callback.abort()
    ...
    def rename(
        ...
        **kwargs,
        ...
        """ Rename dictionary. """
        msg = _("Rename dictionary '{dict_name}' to '{new_name}'?: ")
        msg = msg.format(dict_name=self.name, new_name=new_name)
        if not self.ask_change_confirmation(msg, force=force, callback=callback):
            return callback.abort()
