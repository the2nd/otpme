...
def get_value_acls(split=False, **kwargs):
    ...
    config_params = config.get_config_parameters("ca")
    if split:
...
def register():
    register_oid()
    config.register_config_parameter(name="max_ca_name_len",
                                    ctype=int,
                                    default_value=64,
    ...
    config.register_recursive_default_acl("site", "+ca")
    config.register_default_acl("unit", "+ca")
    config.register_recursive_default_acl("unit", "+ca")
...
def register_oid():
    ...
    ca_path_re = f'{unit_path_re}[/]{ca_name_re}'
    # An OID is not a path, see accessgroup.py: no leading slash, and
    # the unit part is optional because the read OID has none.
    #ca_oid_re = f'ca|{ca_path_re}'
    ca_oid_re = (f'ca[|]{realm_name_re}[/]{site_name_re}'
                f'([/]{unit_name_re})*[/]{ca_name_re}')
...
def register_backend():
    ...
    ca_dir_extension = "ca"
    def path_getter(ca_oid, ca_uuid):
...
class Ca(OTPmeObject):
    ...
    def create_ca_cert(
        ...
        **kwargs,
        ...
        # Create CA cert.
        if verbose_level > 0:
            msg = _("Generating CA certificate ({key_len} bits).")
        ...
        **kwargs,
        ...
        for serial in revoked_certs:
            try:
                ...
                config.raise_exception()
                msg = _("Problem adding cert '{serial}' to CRL: {e}")
                msg = msg.format(serial=serial, e=e)
                return callback.error(msg)
    ...
    def revoke_cert(
        ...
        **kwargs,
        ...
        if cert_issuer != self.path:
            msg = _("Cert was not issued by this CA: {cert_issuer}")
            msg = msg.format(cert_issuer=cert_issuer)
            return callback.error(msg)
    ...
    def set_cert(
        ...
        if self.key and not self.crl:
            ...
            try:
                ...
                config.raise_exception()
                msg = _("WARNING: Problem generating initial CRL: {e}")
                msg = msg.format(e=e)
                return callback.error(msg)
            finally:
                callback.enable()
        ...
        if verify_acls:
            if not self.verify_acl("delete:object"):
                ...
                if not parent_object.verify_acl(del_acl):
                    ...
                    msg = msg.format(path=parent_object.path)
                    return callback.error(msg, exception=PermissionDenied)
    ...
    def show_config(
        ...
        **kwargs,
        ...
        lines.append(f'CRL_VALIDITY="{crl_validity}"')
        ...
        lines.append(f'LAST_CRL_UPDATE="{last_crl_update}"')
