...
**device_token_types (list)**  
Comma-separated list of the token types the SSO portal creates for this
role's device tokens: **password** and/or **totp**. With more than one
the user chooses per token, the first being the default. Unset means
**password**.  
A **totp** device token is created with its PIN disabled — it goes into
a device that has to produce the OTP on its own — and its secret is
shown once, with a QR code for an authenticator app, when it is
created.  
Object types: role

**max_device_tokens (int)**  
How many device tokens one user may hold in this role, between 1
and 128. Unset means no limit. Once it is reached the SSO portal offers
...
is logged in (client or host, address, token and times), which an
install should hand out deliberately. Only the sessions of the portal's
own site are listed - that is where a login of this portal lives. Ending
...
token the user signs in with, so without this an unlocked browser is
enough to take over the account. The user is sent through the portal's
re-auth page and lands back on the deploy page; how long a re-auth stays
fresh is
**sso_reauth_timeout**.**The**recovery**deploy**(**otpme-trashlink**by**mail)**is**not**covered**—**it**has**no**session**to**prove**anything**with,**the**mail**link**is**the**proof.**The**parameter**is**resolved**per**user;**the**most**specific**match**wins**(user**overrides**unit**overrides**site).**  
Object types: site, unit, user

**deploy_fido2_token_reauth (bool, default: true)**  
The same before a security key is registered on the settings page. Every
one of these enrolls a second way into the account, and the new
credential outlives the session it was created in.  
Object types: site, unit, user

**deploy_passkey_reauth (bool, default: true)**  
The same before a passkey is registered, see
...
**deploy_device_token_reauth (bool, default: true)**  
The same before a device token is added, see
**deploy_fido2_token_reauth**. This is the one whose credential is a
password shown on screen rather than a device the user has to be
holding, so it is also the one an install is most likely to find in the
way.  
Object types: site, unit, user
...
it. The parameter is resolved per user; the most specific match wins
...
**site_admin_blacklist (list)**  
Comma separated list of object OIDs (for example
**user\|myrealm/mysite/myuser**) that members of the **SITE_ADMIN** role
must not reach through their role alone. Being in that role normally
grants access to every object of the site without any ACL check; for a
listed object that shortcut is dropped, and access is decided by the
ACLs on the object like it is for anyone else. An ACL set explicitly on
the object therefore still grants access -- the list withdraws the
blanket permission, it does not override an ACL that was granted on
purpose.  
The list is set as OIDs but stored as UUIDs, so an entry survives a
rename of the object and is not inherited by a new object that later
takes over the name. An OID of an object that does not exist is
rejected. The parameter is read from the site handling the request, and
the same rule applies to it as to **realm**, **site** and **node**
objects, which are protected this way unconditionally: it lives on the
site object, where the role shortcut never applies, so site admins
cannot remove entries from their own blacklist.  
Applies to the object interfaces (management, API). LDAP verifies its
ACLs on its own path and is not affected.  
Object types: site
...
Which backup runs to report on and how. Valid values: *all* (one e-mail
per run), *success* (only successful runs), *error* (only failed runs),
*summary* (one e-mail for all backup runs of the host instead of one per
backup object, with one line per object: name, status, duration and the
backed-up/stored size). The summary is sent as soon as the last running
backup has finished. Objects with a different **backup_report_mail_to**
get their own summary e-mail.  
Object types: site, unit, node, share
